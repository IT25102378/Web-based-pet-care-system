...
public class AutomaticLoginAfterApprovalIntegrationTest {
    ...
    void testAutomaticApprovalDetectionAndLogin() throws Exception {
        ...
        // 2. Poll approval status right after registering -> status is PendingApproval, approved is false
        ...
        // 3. Approval-login must fail while still in PendingApproval (400 Bad Request)
        ...
        // 4. System Administrator approves the applicant
        userService.approveUser(user.getUserId(), "USR-003");

        // 5. Poll approval status now -> status is Active, approved is true!
        mockMvc.perform(get("/api/auth/approval-status")
                        .servletPath("/api")
                        .param("token", approvalToken))
                .andExpect(status().isOk())
        ...
        // 6. Browser performs approval-login -> returns real JWT and User object
        ...
        // 7. Access protected /api/auth/me with the issued JWT -> 200 OK
        ...
        // 8. Invalidation test: the used approvalToken cannot be reused to obtain another JWT
        mockMvc.perform(post("/api/auth/approval-login")
                        .servletPath("/api")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(Map.of("approvalToken", approvalToken))))
